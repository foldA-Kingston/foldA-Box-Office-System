...
<script>
  ...
  onMount(async () => {
    ...
    selectedValue = event.purchasable.ticketClasses.map(tc => ({
      label: `${tc.description} – $${tc.price}`,
      value: tc.id
    }));
  });
...
<svelte:head>
  <title>Edit Individual Event</title>
</svelte:head>
<div>
...
<div class="panel">
  ...
  <div class="inputField">
    <h3>
      Embedded media (e.g. YouTube video). For YouTube Video's, please enter as:
    </h3>
